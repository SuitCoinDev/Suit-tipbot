...
class Soak:
    ...
    async def soak(self, ctx, amount: float):
        ...
        if amount_split == 0:
            await self.bot.say("{} **:warning:{} CNMC is not enough to split between {} users:warning:**".format(ctx.message.author.mention, amount, len_receivers))
            return
